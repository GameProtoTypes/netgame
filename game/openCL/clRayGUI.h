...
#define KEY_LEFT_CONTROL    258//?
#define KEY_RIGHT_CONTROL   260//?
#define KEY_RIGHT_SHIFT     261//?
...
//-------------------------------------------------------------------------------
 ...
 int GetCharPressed(void);         // -- GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()
//-------------------------------------------------------------------------------

// Drawing required functions
//-------------------------------------------------------------------------------
 void DrawRectangle(int x, int y, int width, int height, Color color);        // -- GuiDrawRectangle(), GuiDrawIcon()
...
#if defined(RAYGUI_VARIADIC)
 const char *TextFormat(const char *text, ...);               // Formatting of text with variables to 'embed'
#endif
 const char **TextSplit(const char *text, char delimiter, int *count);    // Split text into multiple strings
 int TextToInteger(const char *text);         // Get integer value from text
 int GetCodepoint(const char *text, int *bytesProcessed);     // Get next codepoint in a UTF-8 encoded text
...
BOOL GuiDropdownBox(Rectangle bounds, const char *text, int *active, BOOL editMode)
{
    ...
    GuiDrawText(&str[0], RAYGUI_CLITERAL(Rectangle){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 6, 10, 10 },
...
// NOTE: Requires  variables: frameCounter
 ...
 const char **GuiTextSplit(const char *text, int *count, int *textRow)
{
    ...
    // all used memory is ... it has some limitations:
    ...
    // NOTE: Those definitions could be externally provided if required
     ...
     const char *result[RAYGUI_TEXTSPLIT_MAX_ITEMS] = { NULL };
     char buffer[RAYGUI_TEXTSPLIT_MAX_TEXT_SIZE] = { 0 };
 ...
 int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE))
    {
        ...
        GuiDrawText(isVertical ? &str3[0] : &str4[0], RAYGUI_CLITERAL(Rectangle){ arrowDownRight.x, arrowDownRight.y, isVertical ? bounds.width : bounds.height, isVertical ? bounds.width : bounds.height },
 ...
 const char *TextFormat(const char *text, ...)
{
     ...
     char buffer[RAYGUI_TEXTFORMAT_MAX_SIZE];
