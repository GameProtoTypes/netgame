...
//-------------------------------------------------------------------------------
 ...
 void DrawTextEx(ALL_CORE_PARAMS, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint);  // -- GuiDrawText()
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
 ...
 void DrawRectangleGradientV(ALL_CORE_PARAMS, int posX, int posY, int width, int height, Color color1, Color color2);  // Draw rectangle vertical gradient
//-------------------------------------------------------------------------------
...
// Window Box control
BOOL GuiWindowBox(ALL_CORE_PARAMS, Rectangle bounds, const char *title)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiStatusBar(ALL_CORE_PARAMS_PASS, statusBar, title); // Draw window header as status bar
...
void GuiGroupBox(ALL_CORE_PARAMS,Rectangle bounds, const char *text)
{
...
// Line control
void GuiLine(ALL_CORE_PARAMS, Rectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (text == NULL) GuiDrawRectangle(ALL_CORE_PARAMS_PASS, RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + bounds.height/2, bounds.width, 1 }, 0, BLANK, color);
    else
    {
        ...
        GuiDrawRectangle(ALL_CORE_PARAMS_PASS, RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + bounds.height/2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, BLANK, color);
        GuiDrawText(ALL_CORE_PARAMS_PASS, text, textBounds, TEXT_ALIGN_LEFT, color);
        GuiDrawRectangle(ALL_CORE_PARAMS_PASS, RAYGUI_CLITERAL(Rectangle){ bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height/2, bounds.width - textBounds.width - RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, BLANK, color);
    }
    //--------------------------------------------------------------------
}

// Panel control
void GuiPanel(ALL_CORE_PARAMS, Rectangle bounds, const char *text)
{
...
Rectangle GuiScrollPanel(ALL_CORE_PARAMS, Rectangle bounds, const char *text, Rectangle content, Vector2 *scroll)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        {
            if (IsMouseButtonDown(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            ...
            if (hasHorizontalScrollBar && (IsKeyDown(ALL_CORE_PARAMS_PASS, KEY_LEFT_CONTROL) || IsKeyDown(ALL_CORE_PARAMS_PASS, KEY_RIGHT_SHIFT))) scrollPos.x += wheelMove*20;
...
BOOL GuiButton(ALL_CORE_PARAMS, Rectangle bounds, const char *text)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            if (IsMouseButtonDown(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) 
                state = STATE_PRESSED;
            else 
                state = STATE_FOCUSED;

            if (IsMouseButtonReleased(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) 
            pressed = true;
        }
    }
    //--------------------------------------------------------------------
...
BOOL GuiLabelButton(ALL_CORE_PARAMS, Rectangle bounds, const char *text)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (IsMouseButtonReleased(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) pressed = true;
        }
    }
    //--------------------------------------------------------------------
...
BOOL GuiToggle(ALL_CORE_PARAMS, Rectangle bounds, const char *text, BOOL active)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        {
            if (IsMouseButtonDown(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            else if (IsMouseButtonReleased(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON))
            {
                state = STATE_NORMAL;
                active = !active;
            }
...
int GuiToggleGroup(ALL_CORE_PARAMS, Rectangle bounds, const char *text, int active)
{
...
BOOL GuiCheckBox(ALL_CORE_PARAMS, Rectangle bounds, const char *text, BOOL checked)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, totalBounds))
        {
            ...
            if (IsMouseButtonReleased(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) checked = !checked;
        }
    }
    //--------------------------------------------------------------------
...
BOOL GuiDropdownBox(ALL_CORE_PARAMS, Rectangle bounds, const char *text, int *active, BOOL editMode)
{
    ...
    if ((state != STATE_DISABLED) && (editMode || !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked) && (itemCount > 1))
    {
        ...
        if (editMode)
        {
            ...
            {
                if (IsMouseButtonPressed(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON) || IsMouseButtonReleased(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) pressed = true;
            }

            // Check if already selected item has been pressed again
            if (CheckCollisionPointRec(mousePoint, bounds) && IsMouseButtonPressed(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) pressed = true;
...
BOOL GuiTextBox(ALL_CORE_PARAMS, Rectangle bounds, char *text, int textSize, BOOL editMode)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            int key = GetCharPressed(ALL_CORE_PARAMS_PASS);      // Returns codepoint as Unicode
            ...
            if (keyCount > 0)
            {
                if (IsKeyPressed(ALL_CORE_PARAMS_PASS, KEY_BACKSPACE))
                {
        ...
        else
        {
            ...
            {
                state = STATE_FOCUSED;
                if (IsMouseButtonPressed(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON)) pressed = true;
            }
        }
    }
    //--------------------------------------------------------------------
...
BOOL GuiSpinner(ALL_CORE_PARAMS, Rectangle bounds, const char *text, int *value, int minValue, int maxValue, BOOL editMode)
{
...
#if defined(RAYGUI_NO_ICONS)
    if (GuiButton(ALL_CORE_PARAMS_PASS,leftButtonBound, "<")) tempValue--;
    if (GuiButton(ALL_CORE_PARAMS_PASS,rightButtonBound, ">")) tempValue++;
#else
    if (GuiButton(ALL_CORE_PARAMS_PASS, leftButtonBound, GuiIconText(ICON_ARROW_LEFT_FILL, NULL))) tempValue--;
    if (GuiButton(ALL_CORE_PARAMS_PASS, rightButtonBound, GuiIconText(ICON_ARROW_RIGHT_FILL, NULL))) tempValue++;
#endif
    ...
    pressed = GuiValueBox(ALL_CORE_PARAMS_PASS, spinner, NULL, &tempValue, minValue, maxValue, editMode);
...
BOOL GuiValueBox(ALL_CORE_PARAMS, Rectangle bounds, const char *text, int *value, int minValue, int maxValue, BOOL editMode)
{
...
BOOL GuiTextBoxMulti(ALL_CORE_PARAMS, Rectangle bounds, char *text, int textSize, BOOL editMode)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            int codepoint = GetCharPressed(ALL_CORE_PARAMS_PASS);
            ...
            {
                if (IsKeyPressed(ALL_CORE_PARAMS_PASS, KEY_ENTER))
                {
                    text[textLength] = '\n';
                    textLength++;
                }
...
float GuiSliderBar(ALL_CORE_PARAMS, Rectangle bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue)
{
    return GuiSliderPro(ALL_CORE_PARAMS_PASS, bounds, textLeft, textRight, value, minValue, maxValue, 0);
}
...
// List View control
int GuiListView(ALL_CORE_PARAMS, Rectangle bounds, const char *text, int *scrollIndex, int active)
{
    int itemCount = 0;
...
int GuiListViewEx(ALL_CORE_PARAMS, Rectangle bounds, const char **text, int count, int *focus, int *scrollIndex, int active)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (useScrollBar)
            {
                int wheelMove = (int)GetMouseWheelMove(ALL_CORE_PARAMS_PASS);
...
// Color Panel control
Color GuiColorPanel(ALL_CORE_PARAMS, Rectangle bounds, const char *text, Color color)
{
...
float GuiColorBarAlpha(ALL_CORE_PARAMS, Rectangle bounds, const char *text, float alpha)
{
...
Color GuiColorPicker(ALL_CORE_PARAMS, Rectangle bounds, const char *text, Color color)
{
...
// Message Box control
int GuiMessageBox(ALL_CORE_PARAMS, Rectangle bounds, const char *title, const char *message, const char *buttons)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (GuiWindowBox(ALL_CORE_PARAMS_PASS, bounds, title)) clicked = 0;

    int prevTextAlignment = GuiGetStyle(ALL_CORE_PARAMS_PASS, LABEL, TEXT_ALIGNMENT);
    GuiSetStyle(ALL_CORE_PARAMS_PASS, LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
    GuiLabel(ALL_CORE_PARAMS_PASS, textBounds, message);
    GuiSetStyle(ALL_CORE_PARAMS_PASS, LABEL, TEXT_ALIGNMENT, prevTextAlignment);

    prevTextAlignment = GuiGetStyle(ALL_CORE_PARAMS_PASS, BUTTON, TEXT_ALIGNMENT);
    GuiSetStyle(ALL_CORE_PARAMS_PASS, BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);

    for (int i = 0; i < buttonCount; i++)
    {
        if (GuiButton(ALL_CORE_PARAMS_PASS, buttonBounds, buttonsText[i])) clicked = i + 1;
        buttonBounds.x += (buttonBounds.width + RAYGUI_MESSAGEBOX_BUTTON_PADDING);
    }

    GuiSetStyle(ALL_CORE_PARAMS_PASS, BUTTON, TEXT_ALIGNMENT, prevTextAlignment);
    //--------------------------------------------------------------------
...
int GuiTextInputBox(ALL_CORE_PARAMS, Rectangle bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, int *secretViewActive)
{
    ...
    int messageInputHeight = (int)bounds.height - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT - GuiGetStyle(ALL_CORE_PARAMS_PASS, STATUSBAR, BORDER_WIDTH) - RAYGUI_TEXTINPUTBOX_BUTTON_HEIGHT - 2*RAYGUI_TEXTINPUTBOX_BUTTON_PADDING;
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (GuiWindowBox(ALL_CORE_PARAMS_PASS, bounds, title)) btnIndex = 0;
    ...
    {
        char stars[17] = "****************\n";
        if (GuiTextBox(ALL_CORE_PARAMS_PASS, RAYGUI_CLITERAL(Rectangle){ textBoxBounds.x, textBoxBounds.y, textBoxBounds.width - 4 - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.height }, 
            ((*secretViewActive == 1) || textEditMode)? text : &stars[0], textMaxSize, textEditMode)) textEditMode = !textEditMode;

        char str1[5] = "#44#\0";
        char str2[5] = "#45#\0";
        *secretViewActive = GuiToggle(ALL_CORE_PARAMS_PASS, RAYGUI_CLITERAL(Rectangle){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y, RAYGUI_TEXTINPUTBOX_HEIGHT, RAYGUI_TEXTINPUTBOX_HEIGHT }, (*secretViewActive == 1)? &str1[0] : &str2[0], *secretViewActive);
    }
    else
    {
        if (GuiTextBox(ALL_CORE_PARAMS_PASS, textBoxBounds, text, textMaxSize, textEditMode)) textEditMode = !textEditMode;
    }
    ...
    for (int i = 0; i < buttonCount; i++)
    {
        if (GuiButton(ALL_CORE_PARAMS_PASS, buttonBounds, buttonsText[i])) btnIndex = i + 1;
        buttonBounds.x += (buttonBounds.width + RAYGUI_MESSAGEBOX_BUTTON_PADDING);
    }

    GuiSetStyle(ALL_CORE_PARAMS_PASS,BUTTON, TEXT_ALIGNMENT, prevBtnTextAlignment);
    //--------------------------------------------------------------------
...
Vector2 GuiGrid(ALL_CORE_PARAMS, Rectangle bounds, const char *text, float spacing, int subdivs)
{
...
void GuiDrawText(ALL_CORE_PARAMS, const char *text, Rectangle bounds, int alignment, Color tint)
{
...
// Gui draw rectangle using default raygui plain style with borders
 void GuiDrawRectangle(ALL_CORE_PARAMS, Rectangle rec, int borderWidth, Color borderColor, Color color)
{
    if (color.a > 0)
    {
        // Draw rectangle filled with color
        DrawRectangle(ALL_CORE_PARAMS_PASS, (int)rec.x, (int)rec.y, (int)rec.width, (int)rec.height, color);
    }

    if (borderWidth > 0)
    {
        // Draw rectangle border lines with color
        DrawRectangle(ALL_CORE_PARAMS_PASS, (int)rec.x, (int)rec.y, (int)rec.width, borderWidth, borderColor);
        DrawRectangle(ALL_CORE_PARAMS_PASS, (int)rec.x, (int)rec.y + borderWidth, borderWidth, (int)rec.height - 2*borderWidth, borderColor);
        DrawRectangle(ALL_CORE_PARAMS_PASS, (int)rec.x + (int)rec.width - borderWidth, (int)rec.y + borderWidth, borderWidth, (int)rec.height - 2*borderWidth, borderColor);
        DrawRectangle(ALL_CORE_PARAMS_PASS, (int)rec.x, (int)rec.y + (int)rec.height - borderWidth, (int)rec.width, borderWidth, borderColor);
    }
}
 ...
 int GuiScrollBar(ALL_CORE_PARAMS, Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    if ((state != STATE_DISABLED) && !GetGui(ALL_CORE_PARAMS_PASS)->guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            // Handle mouse wheel
            int wheel = (int)GetMouseWheelMove(ALL_CORE_PARAMS_PASS);
            ...
            else if (IsMouseButtonDown(ALL_CORE_PARAMS_PASS, MOUSE_LEFT_BUTTON))
            {
                if (!isVertical)
                {
...
// NOTE: This function is only used by GuiColorPicker()
 void DrawRectangleGradientV(ALL_CORE_PARAMS, int posX, int posY, int width, int height, Color color1, Color color2)
{
    Rectangle bounds = { (float)posX, (float)posY, (float)width, (float)height };
    DrawRectangleGradientEx(ALL_CORE_PARAMS_PASS, bounds, color1, color2, color2, color1);
}
