...
inline int MapTileGetRotation(ge_uint tileData) {
...
ge_ubyte MapTileCoordValid(ge_int3 mapcoord, int xybuffer)
{
...
{  
    ...
    ge_uint* fromTileData = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, SHORT3_TO_INT3(prevNode->tileIdx));
    ge_uint* toTileData = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, SHORT3_TO_INT3(node->tileIdx));
 ...
 )
{
    ...
    ge_offsetPtr startNodeOPtr = OFFSET_NULL;
...
ge_ubyte AStarSearch_BFS_Routine(ALL_CORE_PARAMS, AStarSearch_BFS* search, ge_int3 startTile, ge_int3 destTile, int startIterations, ge_offsetPtr futurePathPtr)
{
...
void PeepToPeepInteraction(ALL_CORE_PARAMS, Peep* peep, Peep* otherPeep)
{
    ...
    ge_int dist_Q16 = ge_length_v3_Q16(INT3_ADD(peep->physics.base.pos_Q16, -otherPeep->physics.base.pos_Q16));
...
void MapUpdateShadow(ALL_CORE_PARAMS, int x, int y)
{
    ...
    {       
        ...
        ge_uint finalAttr = (clamp(15-0, 0, 15) << 6);
...
void WalkAndFight(ALL_CORE_PARAMS, Peep* peep)
{
    ...
    if( PeepCurrentOrder(peep) != OFFSET_NULL)
    {
        ...
        if(order->pathToDestPtr != OFFSET_NULL)
        {
            ...
            if(path->processing == false && peep->physics.drive.targetPathNodeOPtr == OFFSET_NULL)
            {
                peep->physics.drive.targetPathNodeOPtr = order->pathToDestPtr;
...
void PeepUpdate(ALL_CORE_PARAMS, Peep* peep)
{
    ...
    if(VECTOR3_EQUAL(peep->mapCoord, maptilecoords) == 0)
    {
        ...
        peep->mapCoord = VECTOR3_CAST(maptilecoords, ge_offsetPtrShort3);

        //printf("a");
            // ge_uint* mapData = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, VECTOR3_CAST(peep->mapCoord, ge_int3));
...
void InventoryGui(GUIID_DEF_ALL, Inventory* inventory)
{
...
bool OrderEntryGui(GUIID_DEF_ALL, Order* order,
...
void CommandCenterMachineGui(GUIID_DEF_ALL, SynchronizedClientState* client, Machine* machine)
{
    GUI_COMMON_WIDGET_START()
    if(!goodStart)
        return ;
            ...
            && (gui->draggedOff == 0) && client->curTool == EditorTools_Create)
            {
                ...
                if (mapCoord.z >= 0 && mapCoord.z < mapDepth-1) 
                {
                    ...
                    if(MapDataGetTile(*tileData) == MapTile_NONE)
                    {
                        ...
                        machine->valid = true;
                        machine->mapTilePtr = VECTOR3_CAST(mapCoordSpawn, ge_offsetPtrShort3);
...
void MapCreateSlope(ALL_CORE_PARAMS, int x, int y)
{
    ...
    ge_uint* data22 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[22]));
    MapTile tile22 = MapDataGetTile(*data22);
    if (tile22 == MapTile_NONE)
    {
        BITSET(*tileData, MapTileFlags_LowCornerBTMRIGHT);
    }
    ge_uint* data24 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[24]));
    MapTile tile24 = MapDataGetTile(*data24);
    if (tile24 == MapTile_NONE)
    {
        BITSET(*tileData, MapTileFlags_LowCornerTPRIGHT);
    }
    ge_uint* data23 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[23]));
    MapTile tile23 = MapDataGetTile(*data23);
    if (tile23 == MapTile_NONE)
    {
        BITSET(*tileData, MapTileFlags_LowCornerBTMLEFT);
    }
    ge_uint* data25 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[25]));
    ...
    ge_uint* data0 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[0]));
    ...
    ge_uint* data1 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[1]));
    ...
    ge_uint* data2 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[2]));
    ...
    ge_uint* data3 = MapGetDataPointerFromCoord(ALL_CORE_PARAMS_PASS, INT3_ADD(mapCoordWhole, staticData->directionalOffsets[3]));
...
void MapCreate(ALL_CORE_PARAMS, int x, int y)
{
    ...
    int i = 0;
            ...
            ge_int perlin_z_Q16 = cl_perlin_2d_Q16(TO_Q16(x), TO_Q16(y), TO_Q16(1) >> 6, 8, 0) ;
...
void game_init_single2(ALL_CORE_PARAMS)
{
    ...
    for (ge_uint p = 0; p < maxPeeps; p++)
    {
        ...
        gameState->peeps[p].mapCoord = VECTOR3_CAST(GE_INT3_WHOLE_Q16(gameState->peeps[p].posMap_Q16), ge_offsetPtrShort3);
...
void ParticleDraw(ALL_CORE_PARAMS, Particle* particle, ge_uint ptr)
{
    float3 drawColor;
...
void game_updatepre1(ALL_CORE_PARAMS)
{
    ...
    cl_ulong chunkSize = (maxPeeps) / GAME_UPDATE_WORKITEMS;
...
void game_update(ALL_CORE_PARAMS)
{
    ...
    for (cl_ulong i = 0; i < chunkSize+1; i++)
    {
        ...
        if (idx < MAX_PARTICLES) {
            Particle* p = &gameState->particles[idx];
...
void game_preupdate_2(ALL_CORE_PARAMS) {
    ...
    for (cl_ulong pi = 0; pi < chunkSize+1; pi++)
    {
        ...
        if (idx < SQRT_MAXSECTORS*SQRT_MAXSECTORS)
        {
            ...
            for(int i = mapSector->chunkStart; (i < mapSector->chunkStart + maxPeeps/16) && i < maxPeeps; i++)
            {   
                Peep* peep;
